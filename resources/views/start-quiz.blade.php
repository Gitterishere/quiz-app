...
<head>
    ...
    <title> Quiz : {{ str_replace('-',' ',$quizName) }}</title>
...
<body>
    ...
    <div class=" bg-gray-100 flex flex-col items-center min-h-screen pt-5">
        ...
        <h1 class=" text-4xl text-center text-orange-400 mb-4 font-bold ">{{ str_replace('-',' ',$quizName) }}
        ...
        @if(session('user'))
        <a type="submit" href="/mcq/{{ session('firstMCQ')->id.'/'.$quizName }}" class="  bg-blue-500 rounded-md px-4 py-2 my-5 text-white text-lg hover:cursor-pointer">Start Quiz</a>
        @else
